...
impl SemVal {
    fn contains_opaque(&self) -> bool {
        match self {
            ...
            SemVal::Map(xs) => xs.iter().any(|(k, v)| k.contains_opaque() || v.contains_opaque()),
            SemVal::Struct(xs) | SemVal::Variant(_, xs) => xs.iter().any(|(_, v)| v.contains_opaque()),
            _ => false,
        }
    }
}
...
impl DiffPolicy {
    fn strict() -> Self { DiffPolicy { unordered_maps: false, unordered_lists: false } }
    fn unordered() -> Self { DiffPolicy { unordered_maps: true, unordered_lists: true } }
}

fn sem_eq(a: &SemVal, b: &SemVal, policy: DiffPolicy) -> bool {
    match (a, b) {
        (SemVal::Map(xa), SemVal::Map(xb)) => {
            xa.len() == xb.len() && if policy.unordered_maps {
                multiset_eq_pairs(xa, xb, policy)
            } else {
                xa.iter().zip(xb).all(|((ka, va), (kb, vb))| sem_eq(ka, kb, policy) && sem_eq(va, vb, policy))
            }
        }
        (SemVal::List(xa), SemVal::List(xb)) => {
            xa.len() == xb.len() && if policy.unordered_lists {
        ...
        (SemVal::Struct(xa), SemVal::Struct(xb)) => {
            xa.len() == xb.len()
                && xa.iter().zip(xb).all(|((na, va), (nb, vb))| na == nb && sem_eq(va, vb, policy))
        }
...
fn multiset_eq(a: &[SemVal], b: &[SemVal], policy: DiffPolicy) -> bool {
    ...
    for x in a {
        let Some(j) = b.iter().enumerate().position(|(j, y)| !used[j] && sem_eq(x, y, policy)) else { return false };
        used[j] = true;
    }
    true
}
...
fn multiset_eq_pairs(a: &[(SemVal, SemVal)], b: &[(SemVal, SemVal)], policy: DiffPolicy) -> bool {
    let mut used = vec![false; b.len()];
    for (ka, va) in a {
        let Some(j) = b.iter().enumerate().position(|(j, (kb, vb))| !used[j] && sem_eq(ka, kb, policy) && sem_eq(va, vb, policy)) else { return false };
        used[j] = true;
    }
    true
}
...
fn diff_members(a: &SemVal, b: &SemVal, xa: &[(&'static str, SemVal)], xb: &[(&'static str, SemVal)], policy: DiffPolicy, path: &mut String) -> Option<Diff> {
    if xa.len() != xb.len() { return Some(diff_here(a, b, path)); }
    for ((na, va), (nb, vb)) in xa.iter().zip(xb) {
        if na != nb { return Some(diff_here(a, b, path)); }
        let len = path.len();
        if !path.is_empty() { path.push('.'); }
        path.push_str(na);
        if let Some(d) = first_diff_at(va, vb, policy, path) { return Some(d); }
        path.truncate(len);
    }
    None
}
...
fn first_diff_at(a: &SemVal, b: &SemVal, policy: DiffPolicy, path: &mut String) -> Option<Diff> {
    match (a, b) {
        ...
        (SemVal::Variant(ta, xa), SemVal::Variant(tb, xb)) => {
            if ta != tb { return Some(diff_here(a, b, path)); }
        ...
        (SemVal::List(xa), SemVal::List(xb)) => {
            ...
            for (i, (x, y)) in xa.iter().zip(xb).enumerate() {
                ...
                if let Some(d) = first_diff_at(x, y, policy, path) { return Some(d); }
                path.truncate(len);
            }
            None
        }
        (SemVal::Map(xa), SemVal::Map(xb)) => {
            ...
            for (i, ((ka, va), (kb, vb))) in xa.iter().zip(xb).enumerate() {
                ...
                if let Some(d) = first_diff_at(ka, kb, policy, path) { return Some(d); }
                path.truncate(len);
                path.push_str(&format!("[{i}]"));
                if let Some(d) = first_diff_at(va, vb, policy, path) { return Some(d); }
                path.truncate(len);
...
fn similarity(a: &SemVal, b: &SemVal) -> usize {
    match (a, b) {
        (SemVal::Struct(xa), SemVal::Struct(xb)) | (SemVal::Variant(_, xa), SemVal::Variant(_, xb)) => {
            xa.iter().zip(xb).filter(|((na, va), (nb, vb))| na == nb && va == vb).count()
        }
        _ => usize::from(a == b),
    }
}
...
fn unordered_seq_diff(xa: &[SemVal], xb: &[SemVal], policy: DiffPolicy, path: &str) -> Option<Diff> {
    let mut used_b = vec![false; xb.len()];
    let mut rem_a: Vec<&SemVal> = Vec::new();
    for x in xa {
        match xb.iter().enumerate().position(|(j, y)| !used_b[j] && sem_eq(x, y, policy)) {
            Some(j) => used_b[j] = true,
            None => rem_a.push(x),
        }
    }
    let rem_b: Vec<&SemVal> = xb.iter().enumerate().filter(|(j, _)| !used_b[*j]).map(|(_, y)| y).collect();
    diff_leftovers(&rem_a, &rem_b, policy, path)
}

/// Same as [`unordered_seq_diff`] but for map (key, value) pairs.
fn unordered_pair_diff(xa: &[(SemVal, SemVal)], xb: &[(SemVal, SemVal)], policy: DiffPolicy, path: &str) -> Option<Diff> {
    let pair = |(k, v): &(SemVal, SemVal)| SemVal::Struct(vec![("key", k.clone()), ("value", v.clone())]);
    let a: Vec<SemVal> = xa.iter().map(pair).collect();
    let b: Vec<SemVal> = xb.iter().map(pair).collect();
    unordered_seq_diff(&a, &b, policy, path)
}

fn diff_leftovers(rem_a: &[&SemVal], rem_b: &[&SemVal], policy: DiffPolicy, path: &str) -> Option<Diff> {
    match (rem_a.first(), rem_b.first()) {
        (None, None) => None,
        (Some(x), None) => Some(Diff { path: format!("{path}[+ours({} extra)]", rem_a.len()), ours: short(x), theirs: "<absent>".into() }),
        (None, Some(y)) => Some(Diff { path: format!("{path}[+theirs({} extra)]", rem_b.len()), ours: "<absent>".into(), theirs: short(y) }),
        (Some(x), Some(_)) => {
...
const ARTIFACT_FRAGMENTS: &[&str] = &[
    "trailing_u32", "next_filter", "RandomAppearanceMorph",
    "AugmentationParticles", "FlourishParticles",
...
impl BinLedger {
    fn record(&mut self, class: Class, ty: &str, name: &str, detail: Option<String>) {
        *self.tally.entry(class).or_default() += 1;
        if class == Class::Bug && let Some(d) = &detail {
        ...
        if self.samples.iter().filter(|s| s.0 == class).count() < cap {
            self.samples.push((class, ty.into(), name.into(), detail.unwrap_or_default()));
        }
    }
    ...
    fn record_subdef(&mut self, class: Class, tag: &str, parent: &str, detail: Option<String>) {
        *self.subdef_tally.entry(class).or_default() += 1;
        if class == Class::Bug || class == Class::AcceptArtifact || class == Class::AcceptSort {
            *self.subdef_class_tags.entry(class).or_default().entry(tag.to_string()).or_default() += 1;
        }
        if class == Class::AcceptArtifact && let Some(ref d) = detail {
            let field = d.split(" :: ").next().unwrap_or(d);
            *self.subdef_artifact_paths.entry(format!("{tag}.{field}")).or_default() += 1;
        }
        let cap = if class == Class::Bug { 40 } else { 4 };
        if self.subdef_samples.iter().filter(|s| s.0 == class).count() < cap {
            self.subdef_samples.push((class, tag.into(), parent.into(), detail.unwrap_or_default()));
        }
    }

    fn count(&self, class: Class) -> usize { self.tally.get(&class).copied().unwrap_or(0) }
    fn subdef_count(&self, class: Class) -> usize { self.subdef_tally.get(&class).copied().unwrap_or(0) }

    fn render(&self, title: &str) -> String {
        use std::fmt::Write;
        let mut s = String::new();
        let _ = writeln!(s, "=== VERIFY LEDGER ({title}) ===");
        let order = [Class::Reproduced, Class::AcceptSort, Class::OpaqueOnly, Class::AcceptArtifact, Class::Bug, Class::NonGame, Class::Missing];
        let total: usize = self.tally.values().sum();
        for c in order { let _ = writeln!(s, "  {:<16} {}", format!("{c:?}"), self.count(c)); }
        ...
        if !self.bug_paths.is_empty() {
            ...
            for (path, n) in v.iter().take(30) { let _ = writeln!(s, "       {n:4}  {path}"); }
        }
        if !self.opaque_paths.is_empty() {
            let _ = writeln!(s, "  -- OpaqueOnly blind spots (type.field the differ can't see -> count):");
            let mut v: Vec<_> = self.opaque_paths.iter().collect();
            v.sort_by_key(|(_, n)| std::cmp::Reverse(**n));
            for (path, n) in v.iter().take(30) { let _ = writeln!(s, "       {n:5}  {path}"); }
        }
        let bug_samples: Vec<_> = self.samples.iter().filter(|s| s.0 == Class::Bug).collect();
        if !bug_samples.is_empty() {
            let _ = writeln!(s, "  -- BUG samples:");
            for (_c, ty, name, detail) in bug_samples.iter().take(20) { let _ = writeln!(s, "     {ty} {name} :: {detail}"); }
        }
        let sub_total: usize = self.subdef_tally.values().sum();
        if sub_total > 0 {
            let _ = writeln!(s, "  -- SUB-DEFS (by parent+tag): ");
            for c in [Class::Reproduced, Class::AcceptSort, Class::OpaqueOnly, Class::AcceptArtifact, Class::Bug] {
                let _ = writeln!(s, "     {:<14} {}", format!("{c:?}"), self.subdef_count(c));
            }
            let _ = writeln!(s, "     (of which count-mismatch: {})", self.subdef_count_mismatch);
            if !self.subdef_bug_tags.is_empty() {
                let mut v: Vec<_> = self.subdef_bug_tags.iter().collect();
                v.sort_by_key(|(_, n)| std::cmp::Reverse(**n));
                let _ = writeln!(s, "     -- sub-def Bug tags:");
                for (tag, n) in v.iter().take(20) { let _ = writeln!(s, "        {n:4}  {tag}"); }
            }
            ...
            if !self.subdef_artifact_paths.is_empty() {
                ...
                for (path, n) in v.iter().take(20) { let _ = writeln!(s, "        {n:4}  {path}"); }
            }
            for &cc in [Class::AcceptArtifact, Class::AcceptSort].iter() {
                ...
                if !aa_samples.is_empty() {
                    ...
                    for (_c, tag, parent, detail) in aa_samples.iter().take(10) { let _ = writeln!(s, "        {parent} <{tag}> :: {detail}"); }
                }
            }
            let sub_bugs: Vec<_> = self.subdef_samples.iter().filter(|s| s.0 == Class::Bug).collect();
            if !sub_bugs.is_empty() {
                let _ = writeln!(s, "     -- sub-def Bug samples:");
                for (_c, tag, parent, detail) in sub_bugs.iter().take(15) { let _ = writeln!(s, "        {parent} <{tag}> :: {detail}"); }
            }
        }
        s
    }
...
fn resolver_maps(bin: &DefBinary, names: &Names) -> (HashMap<i32, String>, HashMap<i32, String>) {
    ...
    for e in bin.entries(names) {
        if let Some(fname) = e.file_name && !fname.starts_with("NULLDEF_") {
            idx_to_name.insert(e.global_index as i32, fname.to_string());
        }
    }
    let off_to_str: HashMap<i32, String> = names.map.iter().map(|(off, entry)| (*off as i32, entry.string.clone())).collect();
    (idx_to_name, off_to_str)
}

fn verify_binary(ours: &DefBinary, ours_names: &Names, retail: &DefBinary, retail_names: &Names) -> BinLedger {
    let (ours_idx, ours_off) = resolver_maps(ours, ours_names);
    let (retail_idx, retail_off) = resolver_maps(retail, retail_names);
    let ours_res = Resolvers { def_index_name: &|i| ours_idx.get(&i).cloned(), def_string_value: &|o| ours_off.get(&o).cloned() };
    let retail_res = Resolvers { def_index_name: &|i| retail_idx.get(&i).cloned(), def_string_value: &|o| retail_off.get(&o).cloned() };

    let retail_by_name: HashMap<&str, (&str, &DefBody)> = retail.entries(retail_names)
        .filter_map(|e| match e.file_name { Some(f) if !f.starts_with("NULLDEF_") => Some((f, (e.def_name.unwrap_or("?"), &e.record.body))), _ => None })
        .collect();
    let ours_names_set: std::collections::HashSet<&str> = ours.entries(ours_names)
        .filter_map(|e| e.file_name.filter(|f| !f.starts_with("NULLDEF_"))).collect();

    let mut ledger = BinLedger::default();
    for (&fname, &(ty, _)) in &retail_by_name {
        if !ours_names_set.contains(fname) { ledger.record(Class::Missing, ty, fname, None); }
    }
    for e in ours.entries(ours_names) {
        let Some(fname) = e.file_name else { continue };
        if fname.starts_with("NULLDEF_") { continue; }
        let ty = e.def_name.unwrap_or("?");
        let Some(&(_, theirs)) = retail_by_name.get(fname) else { continue };
        let (Some(mut a), Some(mut b)) = (Some(e.record.body.clone()), Some(theirs.clone())) else { ledger.record(Class::NonGame, ty, fname, None); continue; };
        ...
        let class = if sem_eq(&sa, &sb, DiffPolicy::strict()) {
            if sa.contains_opaque() { Class::OpaqueOnly } else { Class::Reproduced }
        } else if sem_eq(&sa, &sb, DiffPolicy::unordered()) { Class::AcceptSort }
        else {
        ...
        let detail = if class == Class::Bug { first_diff(&sa, &sb, DiffPolicy::unordered()).map(|d| format!("{} :: ours={} theirs={}", d.path, d.ours, d.theirs)) } else { None };
        if class == Class::OpaqueOnly && let Some(p) = first_opaque_path(&sa, &mut String::new()) {
            *ledger.opaque_paths.entry(format!("{ty}.{p}")).or_default() += 1;
        }
        ledger.record(class, ty, fname, detail);
    }
    verify_subdefs(ours, ours_names, retail, retail_names, &ours_res, &retail_res, &mut ledger);
    ledger
}

fn index_bodies(bin: &DefBinary, names: &Names) -> HashMap<u32, (String, DefBody)> {
    bin.entries(names).map(|e| (e.global_index as u32, (e.def_name.unwrap_or("?").to_string(), e.record.body.clone()))).collect()
}

fn verify_subdefs(ours: &DefBinary, ours_names: &Names, retail: &DefBinary, retail_names: &Names, ours_res: &Resolvers, retail_res: &Resolvers, ledger: &mut BinLedger) {
    let ours_bodies = index_bodies(ours, ours_names);
    let retail_bodies = index_bodies(retail, retail_names);
    let retail_subs: HashMap<&str, &Vec<SubDefRecord>> = retail.entries(retail_names)
        .filter_map(|e| match (e.file_name, e.record.sub_defs.as_ref()) { (Some(f), Some(sd)) if !f.starts_with("NULLDEF_") && !sd.is_empty() => Some((f, sd)), _ => None })
        .collect();

    let group = |sd: &[SubDefRecord], bodies: &HashMap<u32, (String, DefBody)>, res: &Resolvers| -> (HashMap<u32, Vec<SemVal>>, HashMap<u32, String>) {
        ...
        for rec in sd {
            if let Some((cls, body)) = bodies.get(&rec.def_index) {
                ...
                by_crc.entry(rec.name_crc).or_default().push(game_body_to_semval(&mut b, res));
    ...
    for e in ours.entries(ours_names) {
        let Some(fname) = e.file_name else { continue };
        if fname.starts_with("NULLDEF_") { continue; }
        let Some(ours_sd) = e.record.sub_defs.as_ref().filter(|s| !s.is_empty()) else { continue };
        let Some(&retail_sd) = retail_subs.get(fname) else { continue };
        ...
        for crc in crcs {
            ...
            let tag = ol.get(&crc).or_else(|| rl.get(&crc)).cloned().unwrap_or_default();
            if o.len() != r.len() {
                ledger.subdef_count_mismatch += 1;
                ledger.record_subdef(Class::Bug, &tag, fname, Some(format!("count ours={} theirs={}", o.len(), r.len())));
                continue;
            }
...
fn classify_pair(sa: &SemVal, sb: &SemVal) -> (Class, Option<String>) {
    if sem_eq(sa, sb, DiffPolicy::strict()) {
        return (if sa.contains_opaque() { Class::OpaqueOnly } else { Class::Reproduced }, None);
    }
    if sem_eq(sa, sb, DiffPolicy::unordered()) { return (Class::AcceptSort, None); }
    match first_diff(sa, sb, DiffPolicy::unordered()) {
        Some(d) if is_artifact_leaf(&d.path) => (Class::AcceptArtifact, Some(d.path)),
        Some(d) => (Class::Bug, Some(format!("{} :: ours={} theirs={}", d.path, d.ours, d.theirs))),
        None => (Class::Bug, None),
    }
}
...
fn pp(v: &SemVal, indent: usize, out: &mut String) {
    ...
    match v {
        SemVal::List(xs) => {
            out.push_str("[\n");
            for (i, x) in xs.iter().enumerate() { out.push_str(&format!("{pad}  [{i}] ")); pp(x, indent + 1, out); out.push('\n'); }
            out.push_str(&format!("{pad}]"));
        }
        SemVal::Struct(xs) | SemVal::Variant(_, xs) => {
            out.push_str("{\n");
            for (name, x) in xs { out.push_str(&format!("{pad}  {name}: ")); pp(x, indent + 1, out); out.push('\n'); }
            out.push_str(&format!("{pad}}}"));
        }
        SemVal::Map(xs) => {
            out.push_str("map{\n");
            for (k, x) in xs { out.push_str(&format!("{pad}  ")); pp(k, indent + 1, out); out.push_str(" => "); pp(x, indent + 1, out); out.push('\n'); }
            out.push_str(&format!("{pad}}}"));
        }
...
fn dump_subdef(_dir: &str, _bin: &str, parent: &str, tag_crc: Option<u32>, names: &Names, b: &DefBinary) -> Vec<(u32, String)> {
    let (idx, off) = resolver_maps(b, names);
    let res = Resolvers { def_index_name: &|i| idx.get(&i).cloned(), def_string_value: &|o| off.get(&o).cloned() };
    let bodies = index_bodies(b, names);
    let mut result = Vec::new();
    for e in b.entries(names) {
        if e.file_name != Some(parent) { continue; }
        let Some(sd) = e.record.sub_defs.as_ref() else { continue };
        for rec in sd.iter() {
            if let Some(tc) = tag_crc && tc != rec.name_crc { continue; }
...
fn run_ledger(ours_dir: &str, retail_dir: &str) {
    ...
    let (Ok(on), Ok(rn)) = (on, rn) else { eprintln!("cannot load names.bin"); return; };
    for (file, title) in [("game", "game"), ("frontend", "frontend"), ("script", "script")] {
...
fn run_dump_subdef(ours_dir: &str, retail_dir: &str, parent: &str, tag: Option<&str>) {
    ...
    for dir in [ours_dir, retail_dir] {
        ...
        for bin in ["game", "frontend", "script"] {
            ...
            for (crc, s) in &d { println!("-- tag crc {crc:#010x} --\n{s}\n"); }
        }
    }
}
...
fn subdef_semvals(parent: &str, tag_crc: Option<u32>, names: &Names, b: &DefBinary) -> Vec<(u32, SemVal)> {
    let (idx, off) = resolver_maps(b, names);
    let res = Resolvers { def_index_name: &|i| idx.get(&i).cloned(), def_string_value: &|o| off.get(&o).cloned() };
    let bodies = index_bodies(b, names);
    let mut out = Vec::new();
...
fn multiset_leftovers<'a>(a: &'a [SemVal], b: &'a [SemVal]) -> (Vec<&'a SemVal>, Vec<&'a SemVal>) {
    ...
    for x in a {
        match b.iter().enumerate().position(|(j, y)| !used_b[j] && sem_eq(x, y, policy)) {
            Some(j) => used_b[j] = true,
            None => only_a.push(x),
        }
    }
    let only_b = b.iter().enumerate().filter(|(j, _)| !used_b[*j]).map(|(_, y)| y).collect();
    (only_a, only_b)
}
...
fn print_setdiff(sa: &SemVal, sb: &SemVal, path: &str) {
    let policy = DiffPolicy::unordered();
    match (sa, sb) {
        (SemVal::Struct(xa), SemVal::Struct(xb)) | (SemVal::Variant(_, xa), SemVal::Variant(_, xb)) if xa.len() == xb.len() => {
            for ((na, va), (_, vb)) in xa.iter().zip(xb) {
                if !sem_eq(va, vb, policy) { print_setdiff(va, vb, &format!("{path}.{na}")); }
            }
        }
        (SemVal::List(xa), SemVal::List(xb)) => {
            let (only_a, only_b) = multiset_leftovers(xa, xb);
            println!("=== {path}: len ours={} theirs={} | {} ours-only, {} theirs-only ===", xa.len(), xb.len(), only_a.len(), only_b.len());
            for x in &only_a { let mut s = String::new(); pp(x, 0, &mut s); println!("  [OURS-ONLY] {s}"); }
            for y in &only_b { let mut s = String::new(); pp(y, 0, &mut s); println!("  [THEIRS-ONLY] {s}"); }
        }
        (SemVal::Map(xa), SemVal::Map(xb)) => {
            let pair = |(k, v): &(SemVal, SemVal)| SemVal::Struct(vec![("key", k.clone()), ("value", v.clone())]);
            let a: Vec<SemVal> = xa.iter().map(pair).collect();
...
fn run_setdiff(ours_dir: &str, retail_dir: &str, parent: &str, tag: Option<&str>) {
    ...
    for bin in ["game", "frontend", "script"] {
        ...
        if ours.is_empty() && retail.is_empty() { continue; }
        // Pair by tag crc in order of appearance.
        for ((crc_o, sa), (_, sb)) in ours.iter().zip(&retail) {
            if sem_eq(sa, sb, DiffPolicy::unordered()) { continue; }
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("usage: verify <ours_dir> <retail_dir> [--dump-subdef|--setdiff <Parent> [<Tag>]]");
        std::process::exit(2);
    }
    ...
    match args.get(3).map(|s| s.as_str()) {
        Some("--dump-subdef") => {
            ...
            run_dump_subdef(ours_dir, retail_dir, parent, args.get(5).map(|s| s.as_str()));
        }
        Some("--setdiff") => {
            let parent = args.get(4).expect("--setdiff requires <Parent>");
            run_setdiff(ours_dir, retail_dir, parent, args.get(5).map(|s| s.as_str()));
        }
